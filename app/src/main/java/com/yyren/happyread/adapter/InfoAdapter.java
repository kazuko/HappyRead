...
public class InfoAdapter {
    ...
    public List<Map<String, Object>> getNoticeData(){
        int icon[] = new int[]{R.mipmap.notice_comment,R.mipmap.notice_deal,R.mipmap.notice_wang,R.mipmap.notice_mail,R.mipmap.notice_like,R.mipmap.notice_more};
        String name[] = new String[]{"评论","交易","书友交流","投稿请求","喜欢和赞","其他提醒"};
        int count[] = new int[]{2,-1,2,3,2,-1};
        for(int i=0;i<icon.length;i++){
            Map<String,Object> map = new HashMap<String,Object>();
            map.put("icon",icon[i]);
            map.put("name",name[i]);
